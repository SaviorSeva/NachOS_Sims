...
int OpenFile::Read(char *into, int numBytes){
    int result = ReadAt(into, numBytes, seekPosition, 1);
    seekPosition += result;
    return result;
}

int OpenFile::Write(const char *into, int numBytes){
    int result = WriteAt(into, numBytes, seekPosition, 1);
    seekPosition += result;
    return result;
}
...
int OpenFile::ReadAt(char *into, int numBytes, int position, int mode){
    ...
    for (i = firstSector; i <= lastSector; i++){
        synchDisk->ReadSector(hdr->ByteToSector(i * SectorSize, mode), &buf[(i - firstSector) * SectorSize]);
...
int OpenFile::WriteAt(const char *from, int numBytes, int position, int mode){
    int fileLength = hdr->FileLength();
    int i, firstSector, lastSector, numSectors;
    ...
    if (!firstAligned)
        ReadAt(buf, SectorSize, firstSector * SectorSize, mode);
    if (!lastAligned && ((firstSector != lastSector) || firstAligned))
        ReadAt(&buf[(lastSector - firstSector) * SectorSize],
               SectorSize, lastSector * SectorSize, mode);
    ...
    for (i = firstSector; i <= lastSector; i++)
        synchDisk->WriteSector(hdr->ByteToSector(i * SectorSize, mode), &buf[(i - firstSector) * SectorSize]);
    delete[] buf;
    return numBytes;
}
...
int OpenFile::insertOpenFileTable(){
    int i = getEmptyIndex();
    if(i==-1) return false;
    openfileTable[i] = this;
    return i; 
}