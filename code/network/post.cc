...
bool MailBox::NoBlockGet(PacketHeader *pktHdr, MailHeader *mailHdr, char *data){
    Mail *mail = (Mail *)messages->NoBlockRemove(); // remove message from list;
                                             // will wait if list is empty
...
bool PostOffice::NoBlockReceive(int box, PacketHeader *pktHdr,
                         MailHeader *mailHdr, char *data){
    ASSERT((box >= 0) && (box < numBoxes));

    bool status = boxes[box].NoBlockGet(pktHdr, mailHdr, data);
    if(status)
        ASSERT(mailHdr->length <= MaxMailSize);
    return status;
}
